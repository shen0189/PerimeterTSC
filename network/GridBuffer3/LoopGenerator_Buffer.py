...
Edge = np.array(range(96))


"""每个出入口都生成一个线圈，子路网也是"""
with open(r"network\GridBuffer\GridBuffer.add.xml", "w") as routes:
    ...
    #     for lanenum in range(4):
            ...
            for lanenum in range(3):
                print(f'\t<e1Detector id="{int(Edge[i]*1000+lanenum)}_in" lane="{int(Edge[i])}_{lanenum}" pos="1" freq="10.00" file="1.xml"/>',file=routes)
        # for i in range(len(EdgeOut)):
        #     for lanenum in range(4):
                print(f'\t<e1Detector id="{int(Edge[i]*1000+lanenum)}_out" lane="{int(Edge[i])}_{lanenum}" pos="472.2" freq="10.00" file="1.xml"/>',file=routes)
